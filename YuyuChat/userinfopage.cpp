    ...
    ui(new Ui::UserInfoPage)
{
    ...
    qDebug() << "icon is " << icon ;
    QPixmap pixmap(icon);
    QPixmap scaledPixmap = pixmap.scaled( ui->head_label->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation); // 将图片缩放到label的大小
    ui->head_label->setPixmap(scaledPixmap); // 将缩放后的图片设置到QLabel上
    ui->head_label->setScaledContents(true); // 设置QLabel自动缩放图片内容以适应大小
    //获取sex
    ...
    QString sexStr = (sex == 1) ? "男" : "女"; // 假设 1 是男，0 或 2 是女
...
void UserInfoPage::on_up_btn_clicked()
{
    ...
    QPixmap scaledPixmap = image.scaled( ui->head_label->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation); // 将图片缩放到label的大小
    ui->head_label->setPixmap(scaledPixmap); // 将缩放后的图片设置到QLabel上
    ui->head_label->setScaledContents(true); // 设置QLabel自动缩放图片内容以适应大小
