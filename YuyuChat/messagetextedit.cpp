...
void MessageTextEdit::insertImages(const QString &url)
{
    ...
    // 计算文件MD5
    QString fileMd5 = Utils::calculateFileHash(url);
    ...
    QString unique_name = Utils::generateUniqueFileName(origin_name);
...
void MessageTextEdit::insertMsgList(QVector<std::shared_ptr<MsgInfo>> &list, MsgType msgtype, QString text_or_url, QPixmap preview_pix, QString unique_name, uint64_t total_size, QString md5)
{
