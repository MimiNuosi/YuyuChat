    ...
    , ui(new Ui::AuthenFriend),_label_point(2,6)
{
    ...
    connect(ui->label_edit, &QLineEdit::textChanged, this, [=](const QString &text) {
        if (text.isEmpty()) {
            ...
            ui->pushButton->setText("点击添加标签：" + text);
    ...
    // 按钮连接
    connect(ui->cancel_button, &QPushButton::clicked, this, &AuthenFriend::SlotApplyCancel);
    connect(ui->sure_button, &QPushButton::clicked, this, &AuthenFriend::SlotApplySure);
}
...
void AuthenFriend::AddTipLbs(QPushButton* lb, QPoint cur_point, QPoint& next_point, int text_width, int text_height)
{
    lb->move(cur_point);
    lb->show();
...
void AuthenFriend::resetLabels()
{
    ...
    for(const QString& key : _friend_label_keys) {
        auto btn = _friend_labels[key]; // 通过 key 去 map 里拿按钮指针

        // 宽度统计与换行判定
        if( _label_point.x() + btn->width() > max_width) {
            _label_point.setY(_label_point.y() + btn->height() + 6);
            _label_point.setX(2);
        }

        btn->move(_label_point);
        btn->show();

        _label_point.setX(_label_point.x() + btn->width() + 2);
...
void AuthenFriend::SlotApplySure()
{
    ...
    //发送tcp请求给chat server
    emit TcpManager::GetInstance()->sig_send_data(ReqID::ID_AUTH_FRIEND_REQ, jsonData);
