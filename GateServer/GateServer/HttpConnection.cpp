...
void HttpConnection::CheckDeadline()
{
	...
	_deadline.async_wait([self](boost::system::error_code ec) {
		try
		{
			...
			if (ec) {
				std::cout << "deadline error:" << ec.what() << "\n";
				return;
			}
...
{
	auto self(shared_from_this());
	http::async_write(_socket, _response, [self](boost::system::error_code ec, std::size_t bytes_transferred) {
		try
		{
			if (ec) {
				std::cout << "http write error:" << ec.what() << "\n";
				return;
			}
			boost::ignore_unused(bytes_transferred);
...
void HttpConnection::HandleRequest()
{
	...
	if (_request.method() == http::verb::get) {
		...
		if (!success) {
			...
			beast::ostream(_response.body()) << "The resource '" << _request.target() << "' was not found.";
			WriteResponse();
			return;
		}
